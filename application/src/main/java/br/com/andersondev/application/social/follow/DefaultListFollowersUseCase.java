...
/**
 * Lista paginada de seguidores de um usuario (GET /users/{id}/followers, publico).
 * Normaliza paginacao: page &gt;= 0, size entre 1 e 50 (data-strategy secao 3).
 */
public final class DefaultListFollowersUseCase extends ListFollowersUseCase {
    ...
    public DefaultListFollowersUseCase(final FollowGateway followGateway) {
    ...
    public Pagination<UserSummaryOutput> execute(final ListSocialCommand command) {
        ...
        return this.followGateway.findFollowers(userId, page, size).map(UserSummaryOutput::from);
    }
