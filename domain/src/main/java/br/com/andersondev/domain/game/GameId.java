package br.com.andersondev.domain.game;

import br.com.andersondev.domain.shared.Identifier;
import br.com.andersondev.domain.shared.IdUtils;

import java.util.Objects;

/**
 * Identificador imutavel de jogo (UUID v4).
 */
public final class GameId extends Identifier {

    private final String value;

    private GameId(final String value) {
        this.value = Objects.requireNonNull(value, "'GameId' nao pode ser nulo");
    }

    public static GameId unique() {
        return new GameId(IdUtils.uuid());
    }

    public static GameId from(final String value) {
        return new GameId(value);
    }

    @Override
    public String getValue() {
        return this.value;
    }

    @Override
    public boolean equals(final Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        final GameId gameId = (GameId) o;
        return Objects.equals(value, gameId.value);
    }

    @Override
    public int hashCode() {
        return Objects.hash(value);
    }

    @Override
    public String toString() {
        return this.value;
    }
}
